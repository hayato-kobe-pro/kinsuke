...
const { sendSyncMail, sendSystemMail, sendSystemMailBaseOnDomain } = require('./src/sendMail');
...
exports.handler = async (event) => {
  ...
  if (!operation) {
    var html = '<html><head><meta name="google-site-verification" content="LsDcIsjZvwunIIxPgJcZRtq7wQlRLk2e3fm4OR-Kp-4" /></head>' +
...
const watch =  (request) => {
  ...
  return new Promise(function (resolve, reject) {
    var params = {
      ...
      ProjectionExpression:'#id, domain_name, app_id, channel_id, kintone_user_id, kintone_user_code, kintone_user_name, google_refresh_token, google_user_email, kintone_refresh_token, google_calendar_name, google_calendar_id, channel_expires_on, time_zone_user, next_sync_token',
...
const updateKintoneGoogleSetting = async (config) => {
  ...
  var params = {
    ...
    UpdateExpression: "set mapping_fields=:mapping_fields, kintone_client_id=:kintone_client_id, kintone_client_secret=:kintone_client_secret, calendar_plugin=:calendar_plugin, user_info=:user_info, user_email=:user_email, updated_at=:updated_at",
    ExpressionAttributeValues:{
...
const configGoogle = async (data) => {
  console.log('starting config google with data: ',JSON.stringify(data, null, 2))
  ...
  return new Promise(function (resolve, reject) {
    axios(options).then(response => {
      ...
      const option = {
        method: 'GET',
        url: 'https://www.googleapis.com/oauth2/v3/tokeninfo?id_token=' + response.data.id_token
      };
      axios(option).then(response => {
        ...
        var paramsUser = {
          TableName : 'test-kintone-google-users',
          ProjectionExpression:'#id, domain_name, app_id, kintone_user_code, google_user_email',
          FilterExpression: 'domain_name = :domain_name and kintone_user_code = :kintone_user_code',
          ExpressionAttributeNames: {
            '#id': 'id',
          },
          ExpressionAttributeValues: {
        ...
        dynamodb.scan(paramsUser,  function(err, dataDb) {
          if (err) {
            ...
            if (Object.keys(dataDb).length > 0 && dataDb.Items.length > 0) {
              let listEmail = dataDb.Items.filter(x =>  x.app_id != appId).map(x => x.google_user_email);
              ...
              if (listEmail.includes(email)) {
                ...
                if (found){
                    ...
                    updateGoogleUser(found.id, domain, appId, kintoneUserId, kintoneUserCode, kintoneUserName, channelId, channel_expires_on, email, accessToken, refreshToken, google_expires_on, calendarName, timeZoneUser)
              ...
              let id = generateId();
              insertGoogleUser(id, domain, appId, kintoneUserId, kintoneUserCode, kintoneUserName, channelId, channel_expires_on, email, accessToken, refreshToken, google_expires_on, calendarName, timeZoneUser)
              .then(data => resolve(data))
              .catch(err => reject(err));
            }
          }
...
const updateGoogleUser = async (id, domain, appId, kintoneUserId, kintoneUserCode, kintoneUserName, channelId, channel_expires_on, email, accessToken, refreshToken, google_expires_on, calendarName, timeZoneUser) => {
  var params = {
    TableName:'test-kintone-google-users',
    Key:{
      'id': id,
...
const insertGoogleUser = async (id, domain, appId, kintoneUserId, kintoneUserCode, kintoneUserName, channelId, channel_expires_on, email, accessToken, refreshToken, google_expires_on, calendarName, timeZoneUser) => {
  var params = {
    TableName: 'test-kintone-google-users',
    Item: {
      'id': id,
      ...
      if (err) {
        ...
        if (Object.keys(dataConf).length > 0) {
          ...
          axios(options).then(response => {
            ...
            dynamodb.scan(paramsUser, async function(err, dataDb) {
              if (err) {
                sendSystemMailBaseOnDomain({domain, error: err, errorType: errorCode.SYS_01});
                reject(err);
              } else {
                var kintone_expires_on = new Date();
                kintone_expires_on.setHours(kintone_expires_on.getHours() + 1);
                if (Object.keys(dataDb).length > 0 && dataDb.Items.length > 0) {
                  var data = await updateKintoneUser(dataDb.Items[0].id, domain, appId, kintoneUserId, kintoneUserCode, kintoneUserName, accessToken, refreshToken, kintone_expires_on);
                  resolve(data);
                } else {
                  let id = generateId();
                  var data = await insertKintoneUser(id, domain, appId, kintoneUserId, kintoneUserCode, kintoneUserName, accessToken, refreshToken, kintone_expires_on);
                  resolve(data);
                }
              }
            });
...
const updateKintoneUser = async (id, domain, appId, kintoneUserId, kintoneUserCode, kintoneUserName, accessToken, refreshToken, kintone_expires_on) => {
  var params = {
    TableName:'test-kintone-google-users',
    Key:{
      'id': id,
...
const insertKintoneUser = async (id, domain, appId, kintoneUserId, kintoneUserCode, kintoneUserName, accessToken, refreshToken, kintone_expires_on) => {
  var params = {
    TableName: 'test-kintone-google-users',
    Item: {
      'id': id,
              ...
              if (err) {
                ...
                if (Object.keys(data).length > 0 && data.Items.length > 0) {
                  for (let indexData = 0; indexData < data.Items.length; indexData++) {
                    ...
                    if (
                      ( !mappingFields.membersField && userInfo && userInfo.field && typeof record[userInfo.field] !== "undefined" && (
                        (record[userInfo.field]['type'] === "CREATOR" && data.Items[indexData].kintone_user_code ===  record[userInfo.field]['value']['code']) || (record[userInfo.field]['type'] === "USER_SELECT" && record[userInfo.field]['value']. length > 0  && data.Items[indexData].kintone_user_code == record[userInfo.field]['value'][0].code ))
                      ) ||
                      ...
                      getAccesstoken(data.Items[indexData].google_refresh_token).then(function(res) {
                        var accessToken = res;
                        const oAuth2Client = new google.auth.OAuth2(clientId);
                        oAuth2Client.setCredentials(accessToken);
                        ...
                        if (body.type == 'ADD_RECORD') {
                          insertEvent(oAuth2Client, calendarId, event).then(function(res) {
                            resolve(res);
                          }).catch(function(err) {
                          ...
                          getEventByKintoneId(oAuth2Client, calendarId, record['$id'].value).then(function(res) {
                            console.log('update', JSON.stringify(event));
                            updateEvent(oAuth2Client, calendarId, res.id, event).then(function(res) {
                              resolve(res);
                            }).catch(function(err) {
      ...
      if (data.Items.length){
        for (let i = 0; i < records.length; i++){
          ...
          for (let indexData = 0; indexData < data.Items.length; indexData++) {
            ...
            // {
              if (
                ( userInfo['type'] == 'CREATOR' &&  record[userInfo['field']] && data.Items[indexData].kintone_user_code == record[userInfo['field']]['value']['code']) ||
                (record[mappingFields.membersField] && record[mappingFields.membersField].value.length && data.Items[indexData].kintone_user_code == record[mappingFields.membersField].value[0].code)
              ) {
              var kintoneUserCode = data.Items[indexData].kintone_user_code;
              var kintoneUserName = data.Items[indexData].kintone_user_name;
              var calendarId = 'primary';
              ...
              try{
                ...
                try {
                  let getEventByKintoneIdResp = await getEventByKintoneId(oAuth2Client, calendarId, record['$id'].value);
                  try {
                    await deleteEvent(oAuth2Client, calendarId, getEventByKintoneIdResp.id)
                  }catch(err){
                    console.log(JSON.stringify(err, null, 2))
                    sendSyncMail({ domain, errorType: errorCode.SYN_06, emailNoti: dataDb.Item.user_email, idRecordKintone: record.$id.value, subject: record[mappingFields.summary].value});
                    return {
                      statusCode: 400,
                      body: JSON.stringify(err),
                    };
...
const deleteEventKintoneToGoogle = async (body) => {
  var params = {
    TableName : 'test-kintone-google-users',
    ProjectionExpression:'#id, domain_name, app_id, kintone_user_code, google_refresh_token, google_calendar_name, google_calendar_id',
    FilterExpression: 'domain_name = :domain_name and app_id = :app_id and kintone_user_code = :kintone_user_code',
    ExpressionAttributeNames: {
      '#id': 'id',
    },
      ...
      if (err) {
        ...
        if (Object.keys(data).length > 0 && data.Items.length > 0) {
          ...
          getAccesstoken(data.Items[0].google_refresh_token).then(function(res) {
            ...
            const oAuth2Client = new google.auth.OAuth2(clientId);
            oAuth2Client.setCredentials(accessToken);
            getEventByKintoneId(oAuth2Client, calendarId, body.eventId).then(function(res) {
              deleteEvent(oAuth2Client, calendarId, res.id).then(function(res) {
                resolve(res);
              }).catch(function(err) {
                sendSyncMail({ domain: body.domain, errorType: errorCode.SYN_06, emailNoti: body.userEmail, subject: body.title});
                reject(err);
              })
            }).catch(function(err) {
              reject(err);
            ...
            sendSyncMail({ domain: body.domain, errorType: errorCode.SYN_07, emailNoti: body.userEmail});
            reject(err);
          })
        } else {
          reject('error');
...
const checkLoginKintone = async (body) => {
  var params = {
    TableName : 'test-kintone-google-users',
    ProjectionExpression:'#id, domain_name, app_id, kintone_user_code, kintone_refresh_token',
    FilterExpression: 'domain_name = :domain_name and app_id = :app_id and kintone_user_code = :kintone_user_code',
    ExpressionAttributeNames: {
      '#id': 'id',
    },
...
const checkLoginGoogle = async (body) => {
  ...
  var params = {
    TableName : 'test-kintone-google-users',
    ProjectionExpression:'#id, domain_name, app_id, kintone_user_code, google_refresh_token, google_calendar_name, google_user_email, channel_id, google_access_token',
    FilterExpression: 'domain_name = :domain_name and app_id = :app_id and kintone_user_code = :kintone_user_code',
    ExpressionAttributeNames: {
      '#id': 'id',
    },
  ...
  return new Promise(function(resolve, reject) {
    dynamodb.scan(params, function(err, data) {
      if (err) {
        ...
        if (Object.keys(data).length > 0 && data.Items.length > 0 && data.Items[0].google_refresh_token != undefined && data.Items[0].google_refresh_token) {
              ...
              // } else {
                if (data.Items[0].google_calendar_name != 'default') {
                  getAccesstoken(data.Items[0].google_refresh_token).then(function(res) {
                    ...
                    calendar.calendarList.list(function(err, resp) {
                      if (err || resp.data == undefined) {
                        ...
                        if (!existCalendarName) {
                          ...
                          var opt = {
                            method: 'POST',
                            headers: {'Authorization': 'Bearer ' + data.Items[0].google_access_token, 'Content-Type': 'application/json', 'Accept': 'application/json'},
                            data: JSON.stringify(info),
                            url: url
                          };
                          axios(opt).then(response => {
                            var params = {
                              ...
                              UpdateExpression: "set google_calendar_id=:google_calendar_id",
                              ExpressionAttributeValues:{
                            ...
                            sendSyncMail({ domain: body.domain, errorType: errorCode.SYN_08, emailNoti: body.userEmail});
                            reject(err.response);
                          });
                        }
                      }
                    ...
                    var url = 'https://www.googleapis.com/calendar/v3/calendars/' + data.Items[0].google_user_email + '/events/watch';
                    var info = {
                      'id': data.Items[0].channel_id,
                      'type': 'web_hook',
                      'address': urlWatch,
...
const getLoggedGoogle = async (body) => {
  var params = {
    TableName : 'test-kintone-google-users',
    ProjectionExpression:'#id, domain_name, app_id, kintone_user_code, kintone_user_name, google_user_email, google_calendar_name, kintone_refresh_token',
    FilterExpression: 'domain_name = :domain_name and app_id = :app_id',
    ExpressionAttributeNames: {
      '#id': 'id',
    },
...
const listEvents =  async (auth, config) => {
  ...
  var timeZoneUser = config.time_zone_user != undefined ? config.time_zone_user : timeZone;
  var params = {
    TableName: 'test-kintone-google-settings',
    Key: {
      'domain_name': domain,
    ...
    dynamodb.get(params, async function(err, dataDb) {
      if (err) {
        ...
        if (Object.keys(dataDb).length > 0) {
          ...
          lastTime.setMinutes(lastTime.getMinutes(), lastTime.getSeconds() - 10);
          console.log(lastTime.toISOString()); //debug
          // calendar.events.list({
          //   calendarId: calendarId,
          //   updatedMin: lastTime.toISOString(),
          //   maxResults: maxEvent,
          //   singleEvents: true,
          //   showDeleted: true,
          //   orderBy: 'startTime',
          ...
          listAllEvent({
            calendarId: calendarId,
            updatedMin: moment().tz("Asia/Tokyo").add(-20, 'seconds').toISOString(),
            singleEvents: true,
            showDeleted: true,
            orderBy: 'startTime',
          }, calendar)
          .then(events => {  
            ...
            // let tStart = moment().tz("Asia/Tokyo").startOf('day')
            // let tEnd = moment().tz("Asia/Tokyo").endOf('day') 
           
            // events = events.filter(x => {
            //   let tUpdate = moment(x.updated);
            //   if (+tStart <= tUpdate && tUpdate <= tEnd){
            //     return true;
            //   }else{
            //     return false;
            //   }
            ...
            console.log('event length: ', events.length);
              if (events.length) {
                ...
                var arrRecordUpdate = [];
                var arrRecordInsert = [];
                getAccesstokenKintone(domain, kintoneClientId, kintoneClientSecret, kintoneRefreshToken).then(async (response) => {
                  ...
                  for (let indexEvent = 0; indexEvent < events.length; indexEvent++) {
                    if (events[indexEvent].recurringEventId != undefined) {
                      if (arrRepeat[events[indexEvent].recurringEventId] == undefined) {
                        arrRepeat[events[indexEvent].recurringEventId] = uuid();
                      }
                    }
                  }
                  try {
                    ...
                    // sleep(3000).then(() => {
                      for (let indexEvent = 0; indexEvent < events.length; indexEvent++) {
                        console.log('event Gooogle', JSON.stringify(events[indexEvent]));
                        console.log('recurringEventId', events[indexEvent].recurringEventId);
                        if (events[indexEvent].creator && events[indexEvent].creator.email == config.google_user_email &&  !events[indexEvent].recurringEventId) {
                          let syncEvent = syncGoogleToKintone(domain, appId, kintoneAccessToken, events, indexEvent, mappingFields, kintoneUserId, kintoneUserCode, kintoneUserName, calendarPlugin, arrRepeat, dataLogin, timeZoneUser, userInfo).then(data => {
                      ...
                      Promise.all(arrPromise).then(async () => {
                        getAccesstokenKintone(domain, kintoneClientId, kintoneClientSecret, kintoneRefreshToken).then(async (res) => {
                          var kintoneAccessToken = res.access_token;
                          if (arrRecordUpdate.length > 0) {
                            for (let index = 1; index < 2; index++) {
                              let arrUpdate = arrRecordUpdate.filter((element, idx) => idx >= 50*(index-1) && idx < 50*index);
                              var records = {
                                'app': appId,
                                'records': arrUpdate
                              };
                              console.log('arr update', JSON.stringify(arrUpdate));
                              var headers = {'Authorization': `Bearer ${kintoneAccessToken}`, 'Content-Type': 'application/json', 'Accept': 'application/json'};
                              var options = {
                              ...
                              await new Promise(function(res, rej) {
                                request(options, function (error, response, body) {
                                  ...
                                  if (!error && body.records != undefined) {
                                    ...
                                    sendSyncMail({ domain, errorType: errorCode.SYN_02, emailNoti: dataDb.Item.user_email, subject: arrRecordUpdate[0].record[[mappingFields.summary]].value });
                                    rej(error);
                                  }
                                });
                              }).catch(err => {
                              ...
                              if (Math.floor(arrRecordUpdate.length/(50*index)) == 0 || (index*50) == arrRecordUpdate.length) {
                                break;
                              }
                            }
                          }
                          if (arrRecordInsert.length > 0) {
                            for (let index = 1; index < 2; index++) {
                              let arrInsert = arrRecordInsert.filter((element, idx) => idx >= 50*(index-1) && idx < 50*index);
                              var records = {
                                'app': appId,
                                'records': arrInsert
                              }
                              ...
                              await new Promise(function(res, rej) {
                                request(options, async function (error, response, body) {
                                  if (!error && body.ids != undefined && body.ids.length > 0) {
                                    var ids = body.ids;
                                    for (let index = 0; index < ids.length; index++) {
                                      var event = arrRecordInsert[index]['eventGoogle'];
                                      event['extendedProperties'] = {
                                        'private': {
                                          'kintoneRecordId': ids[index]
                                        }
                                      };
                                      await updateEvent(auth, calendarId, event.id, event).then(function(res) {
                                    ...
                                    sendSyncMail({ domain, errorType: errorCode.SYN_01, emailNoti: dataDb.Item.user_email, subject: arrRecordInsert[0][[mappingFields.summary]].value});
                              ...
                              if (Math.floor(arrRecordInsert.length/(50*index)) == 0 || (index*50) == arrRecordInsert.length) {
                                break;
                              }
                            }
                          }
                          if (arrRecordDelete.length > 0) {
                            for (let index = 1; index < 2; index++) {
                              let arrDelete = arrRecordDelete.filter((element, idx) => idx >= 50*(index-1) && idx < 50*index);
                              console.log('delete Ev', JSON.stringify(arrDelete));
                              var query = {
                                "app": appId,
                                "ids": arrDelete
                              }
                              var headers = {'Authorization': `Bearer ${kintoneAccessToken}`, 'Content-Type': 'application/x-www-form-urlencoded'};
                              var options = {
                                url: 'https://' + domain + '/k/v1/records.json',
                                method: 'DELETE',
                                headers: headers,
                              ...
                              await new Promise(function(res, rej) {
                                request(options, function (error, response, body) {
                                  if (!error) {
                                    console.log('detele success', JSON.stringify(body));
                                    res(body);
                                  } else {
                                    sendSyncMail({ domain, errorType: errorCode.SYN_03, emailNoti: dataDb.Item.user_email, subject: titleDeleteError });
                              ...
                              if (Math.floor(arrRecordDelete.length/(50*index)) == 0 || (index*50) == arrRecordDelete.length) {
                                break;
                              }
                            }
                          }
                  ...
                  sendSyncMail({ domain, errorType: errorCode.SYN_07, emailNoti: dataDb.Item.user_email});
                  console.log(err);
                  reject(err);
                });
              } else {
...
const syncGoogleToKintone = async (domain, appId, kintoneAccessToken, events, indexEvent, mappingFields, kintoneUserId, kintoneUserCode, kintoneUserName, calendarPlugin, arrRepeat, dataLogin, timeZoneUser, userInfo) => {
    try {
      ...
      if (kintoneId) {
        ...
        eventData = await getEventKintoneById(domain, appId, kintoneId, kintoneAccessToken);
         
        console.log(eventData);
      }
      if (eventGoogle.status == 'cancelled') {
        if (kintoneId) {
          return {'delete': {id: kintoneId, 'title': eventData ? eventData.record[mappingFields.summary].value : summary}};
        } else {
          return {}
        }
      } else {
        ...
        if (eventData && eventData.record && checkUpdateTime(eventData.record, eventGoogle)) {
          if (
            ...
            console.log('startTime check', eventGoogle.start.dateTime && formatISO8601(new Date(startTimeGoogle)) != startTimeKintone);
            console.log('end time check' , eventGoogle.end.dateTime && formatISO8601(new Date(endTimeGoogle)) != endTimeKintone);
            console.log('start check', eventGoogle.start.date && formatDateISO8601(new Date(eventGoogle.start.date)) != startTimeKintone);
            console.log('end check',eventGoogle.end.date && formatDateISO8601(endDate) != endTimeKintone)
            console.log('sumary check',  summary != eventData.record[mappingFields.summary].value);
            console.log('desciption check',  checkChangeDescription(events, indexEvent, eventData, mappingFields));
            console.log('loacation check',  checkChangeLocation(events, indexEvent, eventData, mappingFields))
            console.log('attten check ',checkChangeAttendees(eventData, mappingFields, attendees));
...
const getEventKintoneById = async (domain, appId, kintoneId, kintoneAccessToken) => {
  return new Promise(function (resolve, reject) {
    var headers = {'Authorization': `Bearer ${kintoneAccessToken}`};
    var options = {
      url: 'https://' + domain + `/k/v1/record.json?app=${appId}&id=${kintoneId}`,
      method: 'GET',
      headers: headers
    };
...
const getAllUserLogin = async (domain, appId) => {
  return new Promise(function (resolve, reject) {
    var params = {
      TableName : 'test-kintone-google-users',
      ProjectionExpression:'#id, domain_name, app_id, kintone_user_id, kintone_user_code, kintone_user_name, google_user_email, google_calendar_name',
      FilterExpression: 'domain_name = :domain_name and app_id = :app_id',
      ExpressionAttributeNames: {
        '#id': 'id',
      },
...
const getAttendees = async (eventGoogle, dataLogin) => {
  ...
  return new Promise(function (resolve, reject) {
    try {
      if (eventGoogle.attendees != undefined && eventGoogle.attendees.length > 0) {
        ...
        for (let i = 0; i < attendeesGoogle.length; i++) {
          for (let index = 0; index < dataLogin.length; index++) {
            if (attendeesGoogle[i].email == dataLogin[index].google_user_email  && attendeesGoogle[i].responseStatus != 'declined') {
              attendees.push({
...
const checkChangeDescription = (events, indexEvent, eventData, mappingFields) => {
  if (mappingFields.description &&
    ((events[indexEvent].description != undefined && events[indexEvent].description != eventData.record[mappingFields.description].value) ||
      (events[indexEvent].description == undefined && eventData.record[mappingFields.description].value)
    )) {
    return true;
  }
  return false;
...
const checkChangeLocation = (events, indexEvent, eventData, mappingFields) => {
  if (mappingFields.location &&
    ((events[indexEvent].location != undefined && events[indexEvent].location != eventData.record[mappingFields.location].value) ||
      (events[indexEvent].location == undefined && eventData.record[mappingFields.location].value)
    )) {
    return true;
  }
  return false;
...
const checkChangeAttendees = (eventData, mappingFields, attendees) => {
  ...
  if (mappingFields.attendees) {
    if (attendees.length != eventData.record[mappingFields.attendees].value.length ||
      (attendees.length == 0 && eventData.record[mappingFields.attendees].value.length > 0)
    ) {
      return true;
    }
